...
func GetLogTextPrefix(calldepth int, now time.Time, format string) string {
	return GetNowTime(now, format) + BLANK + GetCaller(calldepth)
}

func GetNowTime(now time.Time, format string) (str string) {
	if format == "" {
		y, m, d := now.Date()
		h, i, s := now.Clock()
